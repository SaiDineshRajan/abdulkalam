...
public class MyRegRunner {

	public static void main(String[] args) throws Exception 
	{
		File f=new File("way2smsnew1.xls");
		 Workbook rwb=Workbook.getWorkbook(f);
		   ...
		   WritableWorkbook wwb=Workbook.createWorkbook(f,rwb); //0 for sheet1
		   ...
		   MyRegmethods mm=new MyRegmethods();
		   ...
		   try
		   {
			   ...
			   for(int i=1;i<nour1;i++)//from 2nd row(index 1)//on tests
			   {
				   int flag=0;
				   ...
				   {
					   //1st row(index 0) have names of column in sheet 2
					   for(int j=1;j<nour2;j++) // on steps
					   {
						   ...
						   if(tid.equalsIgnoreCase(sid))
						   {
							   ...
							   for(int k=0;k<m.length;k++) //for comparing testid
							   {
							   ...
							   {
								   Label l=new Label(nouc2,i,"failed",cf);
								   wsh1.addCell(l);
							   }
